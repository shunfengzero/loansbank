...
<body data-type="widgets">
					...
					<div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
						<div class="widget am-cf">
							<div class="widget-head am-cf">
								<div class="widget-title  am-cf">投资人列表</div>
	...
	<script>
		$(function() {
			...
			$.ajax({
				type : "post",
				url : "/LoansBank/findUsersInvest?cp=1&id=1",
				dataType : "json",
		...
		function findUsers(cp,id) {
			$.ajax({
				type : "post",
				url : "/LoansBank/findUsersInvest?cp=" + cp+"&id" +id,
				dataType : "json",
				success : function(data) {
					if (data.code == 1) {
						bindData(data.msg, data.page, data.currentPage);
		...
		function editorUser(id) {
			$.ajax({
				type : "post",
				url : "/LoansBank/editorUserInvest?id=" + id,
				dataType : "json",
				success : function(data) {
					if (data.code == 1) {
		...
		function deleteUser(id,cp) {
			$.ajax({
				type : "post",
				url : "/LoansBank/deleteUserInvest?id=" + id + "&cp="+cp,
				dataType : "json",
				success : function(data) {
					if (data.code == 1) {
						bindData(data.msg, data.page, data.currentPage);
						window.location.href="/LoansBank/admin/table-list-invest.jsp";
					} else {
						alert(data.msg);
					}
				}
		...
		function serach() {
			...
			$.ajax({
				type : "post",
				url : "/LoansBank/findApplyUsersInvest",
			...
			$(info)
					.each(
							function() {
								...
								html += "<td>" + this.iMoney + "</td>";
								html += "<td>" + this.investTime + "</td>";
								html += "<td>" + this.investDate + "</td>";
								html += "<td>" + this.investMethod + "</td>";
								html += "<td>" + this.withDrawMethod + "</td>";
								html += "<td>" + this.investstatus + "</td>";
								html += "<td>"
								html += "<div class='tpl-table-black-operation'>";
								
								html +="<a href='showinvest.jsp?id=" + this.id+ "' > <i class='am-icon-trash'></i>"
								+ "查看投资" + "</a>";
